...
class Collection_reanalysis_era5_single_levels_monthly_means(Collection):
    collection_name = 'reanalysis-era5-single-levels-monthly-means'
    valid_values = dict(
...
def download_reanalysis_era5_single_levels_monthly_means(month: OneOrMore[str], product_type: OneOrMore[str], time: OneOrMore[str], variable: OneOrMore[str], year: OneOrMore[str], area_group: Union[FreeEditionType, GeographicExtentType] = 'global', data_format: str = 'grib', download_format: str = 'unarchived'):
    return Collection_reanalysis_era5_single_levels_monthly_means.download(month=month, product_type=product_type, time=time, variable=variable, year=year, area_group=area_group, data_format=data_format, download_format=download_format)
