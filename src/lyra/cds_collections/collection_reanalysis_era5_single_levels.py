...
class Collection_reanalysis_era5_single_levels(Collection):
    collection_name = 'reanalysis-era5-single-levels'
    valid_values = dict(
    ...
    @Collection.wrapper
    def download(cls, day: OneOrMore[str], month: OneOrMore[str], time: OneOrMore[str], variable: OneOrMore[str], year: OneOrMore[str], area_group: Union[FreeEditionType, GeographicExtentType] = 'global', data_format: str = 'grib', download_format: str = 'unarchived', product_type: OneOrMore[str] = 'reanalysis'): 
        """
        Parameters
        ----------
        :param day:
...
def download_reanalysis_era5_single_levels(day: OneOrMore[str], month: OneOrMore[str], time: OneOrMore[str], variable: OneOrMore[str], year: OneOrMore[str], area_group: Union[FreeEditionType, GeographicExtentType] = 'global', data_format: str = 'grib', download_format: str = 'unarchived', product_type: OneOrMore[str] = 'reanalysis'):
    return Collection_reanalysis_era5_single_levels.download(day=day, month=month, time=time, variable=variable, year=year, area_group=area_group, data_format=data_format, download_format=download_format, product_type=product_type)
