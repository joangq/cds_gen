...
class Collection_sis_biodiversity_cmip5_regional(Collection):
    collection_name = 'sis-biodiversity-cmip5-regional'
    valid_values = dict(
    ...
    @Collection.wrapper
    def download(cls, derived_variable: OneOrMore[str], ensemble_member: OneOrMore[str], experiment: OneOrMore[str], model: OneOrMore[str], region: OneOrMore[str], statistic: OneOrMore[str], variable: OneOrMore[str], version: OneOrMore[str] = '1.0'): 
        """
        Parameters
        ----------
...
def download_sis_biodiversity_cmip5_regional(derived_variable: OneOrMore[str], ensemble_member: OneOrMore[str], experiment: OneOrMore[str], model: OneOrMore[str], region: OneOrMore[str], statistic: OneOrMore[str], variable: OneOrMore[str], version: OneOrMore[str] = '1.0'):
    return Collection_sis_biodiversity_cmip5_regional.download(derived_variable=derived_variable, ensemble_member=ensemble_member, experiment=experiment, model=model, region=region, statistic=statistic, variable=variable, version=version)
